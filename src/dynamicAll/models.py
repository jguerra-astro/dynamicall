...
class TestPotential(JaxDensity):
    def __init__(self, rhos: float, rs: float, a: float):
        ...
        self._rs = rs
        self._a = a
        self._M = 2 * jnp.pi * rhos * rs**3
        self.params = {"rhos": self._rhos, "rs": self._rs}
...
class Hernquist(JaxDensity):
    ...
    def density(self, r: Union[float, jax.Array]) -> Union[float, jax.Array]:
        return Hernquist._density(r, self.params)

    def mass(self, r: Union[float, jax.Array]) -> Union[float, jax.Array]:
        return Hernquist._mass(r, self.params)

    def potential(self, r: Union[float, jax.Array]) -> Union[float, jax.Array]:
        return Hernquist._potential(r, self.params)

    def v_circ(self, r: Union[float, jax.Array]) -> Union[float, jax.Array]:
        r"""
        Circular velocity for Hernquist profile

        .. math::
            v_{\rm circ} = \sqrt{\frac{G M(r) r}{r+a}}
    ...
    def dispersion(self, r: Union[float, jax.Array]) -> Union[float, jax.Array]:
        ...
        term2 = 12 * r * (r + self._a) ** 3 * jnp.log((r + self._a) / r) / self._a**4
        term3 = (
            -r
            / (r + self._a)
            * (
    ...
    def logDF(self, E):
        ...
        term1 = (1 - q**2) ** (-5 / 2)
        term2 = 3 * jnp.arcsin(q) + q * (1 - q**2) ** (1 / 2) * (1 - 2 * q**2) * (
            8 * q**4 - 8 * q**2 - 3
        )
        return jnp.log(term1 * term2)
    ...
    def _density(r, params):
        q = r / params["rs"]
        return params["rhos"] * q**-1 * (1 + q) ** -3

    @classmethod
    @partial(jax.jit, static_argnums=(0,))
    def _mass(cls, r, params):
        q = r / params["rs"]
        return 2 * jnp.pi * params["rhos"] * params["rs"] ** 3 * q**2 / (1 + q) ** 2

    @staticmethod
    def _potential(r, params):
        q = r / params["rs"]
        G = Hernquist.G
        return -4 * jnp.pi * G * params["rhos"] * params["rs"] ** 2 / (1 + q) / 2
...
class Jaffe(JaxDensity):
    def __init__(self, rhos: float, rs: float):
        self._rhos = rhos
        self._rs = rs
        self._M = 4 * jnp.pi * rhos * rs**3
        self.params = {"rhos": self._rhos, "rs": self._rs}

    def density(self, r):
        return Jaffe._density(r, self.params)

    def mass(self, r):
        return Jaffe._mass(r, self.params)

    def potential(self, r):
        return Jaffe._potential(r, self.params)

    @staticmethod
    def _density(r, params):
        q = r / params["rs"]
        return params["rhos"] * q**-2 * (1 + q) ** -2

    @staticmethod
    def _mass(r, params):
        q = r / params["rs"]
        return 4 * jnp.pi * params["rhos"] * params["rs"] ** 3 * q / (1 + q)
    ...
    def _potential(r, params):
        ...
        return (
            ...
            * params["rhos"]
            * params["rs"] ** 2
            * jnp.log(r / (r + params["rs"]))
        )

    @staticmethod
    def from_m_rs(M: float, rs: float):
        r"""
        Alternate Constructr for Jaffe profile using total mass and scale radius
...
class NFW(JaxDensity):
    ...
    _dm_priors = {"dm_rhos": dist.Uniform(5, 30), "dm_rs": dist.Uniform(-10, 10)}

    def __init__(self, rhos: float, rs: float, rlim="r200"):
        self._rhos = rhos
        self._rs = rs
        if rlim == "r200":
            self._rlim = self.r200()
            self._M = self.mass(self._rlim)

        elif rlim is None or rlim == jnp.inf:
            print(
                "No limit set on rlim, Note the mass for the NFW profile diverges as r->inf"
            )
        else:
            self._rlim = rlim
    ...
    def cdf(self, r):
        ...
        return self._mass(r, self._rhos, self._rs) / self._mass(
            r_200, self._rhos, self._rs
        )
    ...
    def pdf(self, r):
        ...
        return (
            ...
            * np.pi
            * r**2
            * NFW._density(self._rhos, self._rs, r)
            / NFW._mass(r_200, self._rhos, self._rs)
        )
    ...
    def potential(self, r):
        ...
        G = self._G
        return (
            -4 * jnp.pi * G * self._rhos * self._rs**3 * jnp.log(1.0 + r / self._rs) / r
        )
    ...
    def _mass(r, rhos: float, rs: float):
        ...
        q = r / rs
        mNFW = (
            4 * jnp.pi * rhos * rs**3 * (jnp.log(1 + q) - (r / (rs + r)))
    ...
    def _potential(r, rhos: float, rs: float):
        G = NFW._G
        return -4 * jnp.pi * G * rhos * rs**3 * jnp.log(1.0 + r / rs) / r
    ...
    @staticmethod
    def from_M_delta_c(M_delta, c, delta=200, rhoc=None):
        r"""
        ...
        """
        if rhoc is None:
            # use astropy to get the critical density
            from astropy.cosmology import default_cosmology

            # Get the default cosmology
            cosmo = default_cosmology.get()
            rhoc = cosmo().critical_density(0).to(u.Msun / u.kpc**3).value

            r_delta = (3 * M_delta / (4 * jnp.pi * delta * rhoc)) ** (1 / 3)
            rs = r_delta / c
            rhos = M_delta / (4 * jnp.pi * rs**3 * (jnp.log(1 + c) - c / (1 + c)))
...
class HernquistZhao(JaxDensity):
    ...
    def __init__(
        self, rhos: float, rs: float, a: float, b: float, c: float, rlim="r200"
    ):
        self._rhos = rhos
        self._rs = rs
        self._a = a
        ...
        if rlim == "r200":
            ...
            print(
                "No limit set on rlim, Note the mass for Hernquist-Zhao diverge as r->inf for c >= 3"
            )
        else:
            self._rlim = rlim
    ...
    @staticmethod
    def _mass_test(
        r: float, rhos: float, rs: float, a: float, b: float, c: float
    ) -> float:
        r"""
        ...
        def less_than_rs():
            ...
            coeff = 4 * jnp.pi * r
            mrltrs = coeff * jnp.sum(
                wi
                * jnp.cos(xi)
                * HernquistZhao._density(r * jnp.sin(xi), rhos, rs, a, b, c)
                * (r * jnp.sin(xi)) ** 2
            )
            return mrltrs
        ...
        def greater_than_rs():
            ...
            mrltrs = coeff * jnp.sum(
                wi
                * jnp.cos(xi)
                * HernquistZhao._density(rs * jnp.sin(xi), rhos, rs, a, b, c)
                * (jnp.sin(xi)) ** 2
            )
            ...
            xk = 0.5 * (x3 - x2) * HernquistZhao._xk + 0.5 * (x3 + x2)
            wk = 0.5 * (x3 - x2) * HernquistZhao._wk
            coeff2 = 4 * jnp.pi * rhos * r**3
            mrgtrs = coeff2 * jnp.sum(
                wk
                * jnp.cos(xk)
                * HernquistZhao._density(r * jnp.sin(xk), 1.0, rs, a, b, c)
                * (jnp.sin(xk)) ** 2
            )
...
class gNFW(JaxDensity):
    ...
    def __init__(self, gamma, rhos, rs, rlim="r200"):
        self._gamma = gamma
        self._rhos = rhos
        self._rs = rs
        ...
        if rlim == "r200":
            ...
            print(
                "No limit set on rlim, Note the mass for gNFW profile diverges as r->inf"
            )
        else:
            self._rlim = rlim
...
class Isochrone(JaxDensity):
    ...
    def action_r(self, x, v):
        ...
        term2 = 0.5 * (L + jnp.sqrt(L**2 + 4 * G * self._M * self._b))
        return term1 - term2
    ...
    def peri(self, x0, v0):
        r0 = jnp.linalg.norm(x0, axis=0)
        bisec = Bisection(
            optimality_fun=centre, lower=1e-12, upper=r0, check_bracket=True
        )
    ...
    def apo(self, x0, v0):
        r0 = jnp.linalg.norm(x0, axis=0)
        bisec2 = Bisection(
            optimality_fun=centre, lower=r0, upper=500, check_bracket=True
        )
    ...
    def _DF(w, M, b):
        ...
        def E_lt_zero():
            ...
            f_I = numerator * term1 + numerator * term2 * jnp.arcsin(
                jnp.sqrt(E)
            ) / jnp.sqrt(tE * (1 - tE))
...
class Gaussians(JaxDensity):
    ...
    def density_i(r, M: float, sigma: float):
        rho = (
            M / (jnp.sqrt(2 * jnp.pi) * sigma) ** 3
        )  # part with the units  [Mass * Length**-3]
    ...
    def mass_j(r, M: float, sigma: float):
        return M * (
            jax.scipy.special.erf(r / (jnp.sqrt(2) * sigma))
            - jnp.sqrt(2 / jnp.pi) * r * jnp.exp(-(r**2) / (2 * sigma**2)) / sigma
        )
...
class Plummer(JaxDensity):
    ...
    def potential(self, r: jax.Array) -> jax.Array:
        r"""
        Gravitational potential of Plummer model
    ...
    def sample_w_conditional(
        self, N: int, save=False, fileName="./out.txt"
    ) -> np.ndarray:
        # first sample from r
    ...
    def sample_w(
        self, N: int, evolve=False, save=False, fileName="./out.txt"
    ) -> np.ndarray:
        r"""
        ...
        sampler = emcee.EnsembleSampler(
            nwalkers, ndim, log_probability, args=[self._M, self._a]
        )
    ...
    @staticmethod
    def _density(r: jnp.ndarray, M: float, a: float) -> jnp.ndarray:
        r"""
    ...
    def _projection_function(R: np.ndarray, param_dict) -> np.ndarray:
        ...
        q = R / param_dict["a"]
        coeff = param_dict["M"] / (
            np.pi * param_dict["a"] ** 2
        )  # units [M.unit/a.unit**2]
        return coeff * (1 + q**2) ** (-2)
...
class gEinasto(JaxDensity):
    ...
    @staticmethod
    def _density(
        r: jax.Array, rhos: float, rs: float, alpha: float, gamma: float
    ) -> jax.Array:
        r"""
        Parameters
        ----------
...
class Anisotropy:
    r"""
    Base class for  velocity anisotropy profiles

    The anistropy parameter :math:`\beta(r)` is defined as

    .. math::
        \beta(r) = 1-\frac{\sigma_{t}^{2}}{\sigma_{r}^{2}}

    where :math:`\beta` is the anisotropy parameter, :math:`\sigma_{r}` is the radial velocity dispersion, and :math:`\sigma_{t}` is the tangential velocity dispersion which define as :math:`\sigma_{t}^{2} = \sigma_{\theta}^{2} + \sigma_{\phi}^{2}`.
    It is a measure of the degree of radial vs tangential orbits in a system.
    With line of sight velocities, the anisotropy parameter is not directly observable and poorly constrained.

    :math:`\beta=0` corresponds to isotropic orbits, :math:`\beta< 0` corresponds to tangentially biased orbits, and :math:`\beta>0` corresponds to radially biased orbits.
    :math:`\beta=1` corresponds to purely radial orbits.
    :math:`\beta=-\infty` corresponds to purely tangential orbits.
    ...
    def get_priors(cls):
        r"""
        Retrieve Priors -- used during inference
        """
...
class BetaConstant(Anisotropy):
    ...
    _priors = {"beta_0": dist.Uniform(-5, 1)}
...
class BetaSymmetric(Anisotropy):
    ...
    _prior = {"beta_tilde": dist.Uniform(-1, 1)}
