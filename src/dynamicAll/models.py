...
class Jaffe(JaxPotential):
    ...
    def logDF(self, r):
        ...
        r : _type_
            _description_
        """
        fm = lambda x: 0.5 * jnp.sqrt(jnp.pi) * jnp.exp(x**2) * jax.scipy.special.erf(x)
        # fp
...
class HernquistZhao(JaxPotential):
    ...
    def density(self, r: Union[float, jax.Array]) -> Union[float, jax.Array]:
        ...
        """
        return HernquistZhao._density(
            r, self._rhos, self._rs, self._a, self._b, self._c
        )
...
class NFW(JaxPotential):
    ...
    def potential(self, r):
        ...
        G = self._G
        return (
            -4 * np.pi * G * self._rhos * self._rs**3 * jnp.log(1.0 + r / self._rs) / r
        )
    ...
    def _mass(r, rhos: float, rs: float):
        ...
        q = (rs + r) / rs
        mNFW = (
            4 * jnp.pi * rhos * rs**3 * (jnp.log(q) - (r / (rs + r)))
