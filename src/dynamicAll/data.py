...
class KeckData(Data):
    def __init__(self, table, galaxy: SkyCoord = None, unit=[u.deg, u.deg]):
        ...
        self.table = (
            table  # Don't think we actually need this, but i'll keep i around for now
        )
        self._prob_m = table["Pmem_pure"]  # membership probabilities

        self.member = np.where(self._prob_m == 1)[
            0
        ]  # use probabilities to get rid of everything that is not a member
        ...
        self.d_vlos = table["v_err"][
            self.member
        ]  # line of sight velocity errors in km/s
    ...
    def plot_pos(self, plot_all=True):
        ...
        if plot_all:
            ax.scatter(
                self.table["RA"],
                self.table["DEC"],  # first plot all objects in table
                marker="s",
                alpha=1.0,
                color="black",
                label="all",
            ...
            ax.scatter(
                self.ra,
                self.dec,  # plot members
                alpha=0.8,
                marker="+",
                label="members",
            )
            ax.scatter(
                self.table["RA"][self.non_member],
                self.table["DEC"][self.non_member],
                marker=".",
                # alpha=1,
                # facecolor='none',
                label="non-members",
    ...
    def from_name(table, name):
        ...
        df = galaxies[
            "MAIN_ID", "RA_d", "DEC_d", "Distance_distance", "Distance_unit"
        ].to_pandas()
        ...
        try:
            ...
            warnings.warn(
                "\n Something went wrong, the Name provided was not in our database,",
                stacklevel=2,
            )
        ...
        try:
            ...
            galaxy = SkyCoord(
                ...
                distance=(
                    temp["Distance_distance"] * u.Unit(temp["Distance_unit"][0])
                ).to(u.kpc),
            )
